...
actor_ppo_max_token_len=$(((max_prompt_length + max_response_length)*2 + 512))
infer_ppo_max_token_len=$(((max_prompt_length + max_response_length)*2 + 512))
...
ray start --head --port=$PET_MASTER_PORT --dashboard-host='0.0.0.0' --disable-usage-stats --num-cpus 32 --min-worker-port=10002 --max-worker-port=10501

ray job submit --verbose \
    -- python3 -m recipe.dapo.main_dapo_self_verification_answeronly \
    ...
    +algorithm.verification_interval=${verification_interval} \
    +algorithm.verification_looplen=${verification_looplen} \
