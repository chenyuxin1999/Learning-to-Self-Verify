...
# ! Preliminary 研究：在训练verification 是否可以提升generation_on_policy 版本。
    # ! 这个模型应该不能从 Base 模型开始选择
...
class RayDAPOSelfVerificationTrainer(RayPPOTrainer):
    ...
    #     self.verification_max_token_len_per_gpu = self.config.trainer.verification_max_token_len_per_gpu
    #     self.verification_log_prob_max_token_len_per_gpu = self.config.trainer.verification_log_prob_max_token_len_per_gpu
    #     self.verification_log_prob_max_token_len_per_gpu = self.config.trainer.verification_log_prob_max_token_len_per_gpu
    ...
    def _create_verification_prompts(self, batch: DataProto, mode: str = "judge_answer") -> DataProto:
        ...
        for uid, samples in prompt_groups.items():
            ...
            if (not all(r == 0 for r in group_rewards)) and (not all(r == 1 for r in group_rewards)):
                ...
                # response = self._extract_response_text(batch, selected_idx, prompt_ids)
                # reward_zero_samples.append(
                #     self._create_verification_sample(question, response, 'No', selected_idx, verification_prompt_template)
                # )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if current_generation_batch is not None:
                    # Create verification prompts based on generation results
                    verification_batch = self._create_verification_prompts(current_generation_batch)

                    if verification_batch is not None:
                        with marked_timer("verification_rollout", timing_raw, "green"):
                            # STEP 3: Rollout verification task
                            verification_output = self.actor_rollout_wg.generate_sequences(verification_batch)

                        # Use verification_output as the base for reward computation
                        # It already contains the complete sequences (prompts + responses)
                        verification_batch_for_reward = deepcopy(verification_output)

                        # Add verification metadata to match DAPO reward manager format
                        batch_size = len(verification_batch_for_reward)
                        verification_labels = verification_batch.non_tensor_batch["label"]

                        # Create reward_model metadata for verification data
                        reward_model_data = []
                        for label in verification_labels:
                            reward_model_data.append({
                                "ground_truth": label,  # Yes/No for verification
                                "style": "function"     # Use function-based scoring
                            })
                        ...
                        verification_batch_for_reward.non_tensor_batch["data_source"] = np.array(
                            ["verification"] * batch_size, dtype=object  # Use same scoring as generation
                        )
                        ...
                        # Set up verification batch for training
                        ...
                        # STEP 4: Train verification task
